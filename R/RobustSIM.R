#' RobustSIM
#' @description This function performs a robust inference procedure for high-dimensional single index model with distribution transformation
#' @param x: the data matrix
#' @param y: the response
#' @param interest: coefficient of interest
#' @param outlier.prop: the proportion of outliers
#' @param outlier.multi: a pre-set constant standing for outlier strength
#' @param penalty: the choice of penalty
#' @param nfolds: number of folds for cross validation
#' @return the p-values and test satstistics for coefficients of interest
#' @examples
#' library(Matrix)
#' n <- 200; p <- 400
#' p.sub <- p/10
...
RobustSIM <- function(x, y, interest=c(1:2, p-1, p), 
  ...
  return(list(pval.record=pval.record, stat.record=stat.record))
}